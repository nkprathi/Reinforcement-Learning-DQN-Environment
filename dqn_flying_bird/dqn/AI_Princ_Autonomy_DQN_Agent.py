...
class ContinuousMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 2. Distance Shaping (Encourage moving closer)
        # Scale factor 3.0 => Gentle hint, allowing detours around walls
...
def train_dqn(
    ...
    # No rendering for speed during training
    ...
    for episode in range(num_episodes):
        ...
        for t in range(max_steps_per_episode):
            ...
            if len(replay_buffer) >= min_replay_size:
                ...
                with torch.no_grad():
                    next_q_values = target_dqn(next_states_t).max(dim=1, keepdim=True)[0]
        ...
        print(f"Episode {episode + 1}/{num_episodes}, Reward: {episode_reward:.2f}, Steps: {t + 1}")

    env.close()
    return dqn


def run_and_render(dqn, num_steps=300):
    """Run a single long episode with rendering and your exact print format."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    while step_count < num_steps:
        ...
        # Greedy action from trained DQN
        ...
        # Print in the same format as your random agent
        print(f"Obs: {next_obs}, Reward: {reward}, Done: {done}")

        obs = next_obs
        step_count += 1

        if done:
            # Start a new episode, just like your script
            obs, info = env.reset()
            done = False
...
if __name__ == "__main__":
    ...
    epsilon_decay = 0.99 # Decay per episode (much faster for demo)
